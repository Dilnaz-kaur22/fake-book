...
const sub1 = new Subscriber(2204, 'Dilnaz', 'Kaur', 'dilnazkaur22@gmail.com', 10, 5, false);
...
imgUpload.addEventListener('change', () => {
  ...
  if (allowedExtensions.includes(extension)) { // file type validation via extension
    ...
    infoMsg.innerHTML =
      (`Please upload this following files: ${allowedExtensions.join(', ')}`);
...
postBtn.addEventListener('click', () => {
  ...
  try {
    if (input.toString().length >= 1 || fileResult.innerHTML != '') {
      ...
      postArea.style.border = '1px solid #ff2846';
      infoMsg.innerHTML = 'Please add input to Post :)';
      postArea.appendChild(infoMsg);
    }
